  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      if (reportData && reportData.data) {
        const formattedData: DataType[] = reportData.data.map((item: any) => {
          const statsMap = item.stats_map || {};
          const regions = Object.keys(statsMap);

          let sumDailyOutput = 0;
          let sumTheoreticalPower = 0;
          let sumEffectivePower = 0;
          let sumTotalMachines = 0;
          let sumTotalFailures = 0;
          let sumNewFailures = 0;
          let weightedEfficiencyNumerator = 0;

          const subAccountStats: SubAccountStat[] = regions.map((region) => {
            const power = statsMap[region]?.power_stats || {};
            const machine = statsMap[region]?.machine_stats || {};

            const dailyOutput = Number(power.DailyOutput) || 0;
            const theoreticalPower = Number(power.TheoreticalHashrate) || 0;
            const effectivePower = Number(power.EffectivePower) || 0;
            const efficiency = Number(power.Efficiency) || 0;

            const totalMachines = Number(machine.TotalMachine) || 0;
            const totalFailures = Number(machine.TotalFailure) || 0;
            const newFailures = Number(machine.NewFailure) || 0;
            const newFailureRate =
              machine.NewFailureRate != null
                ? Number(machine.NewFailureRate)
                : totalMachines
                  ? (newFailures / totalMachines) * 100
                  : 0;
            const totalFailuresRate = totalMachines ? (totalFailures / totalMachines) * 100 : 0;

            sumDailyOutput += dailyOutput;
            sumTheoreticalPower += theoreticalPower;
            sumEffectivePower += effectivePower;
            sumTotalMachines += totalMachines;
            sumTotalFailures += totalFailures;
            sumNewFailures += newFailures;
            weightedEfficiencyNumerator += efficiency * effectivePower;

            return {
              pool_name: region,
              btcOutput24h: dailyOutput,
              theoreticalPower,
              power24h: effectivePower,
              effectiveRate24h: efficiency,
              totalMachines,
              totalFailures,
              totalFailuresRate,
              failures24h: newFailures,
              failureRate24h: newFailureRate,
              impactRatio: 0,
              limitImpactRate: 0,
          ...
          const effectiveRate24h = sumEffectivePower ? weightedEfficiencyNumerator / sumEffectivePower : 0;
          const failureRate24h = sumTotalMachines ? (sumNewFailures / sumTotalMachines) * 100 : 0;

          return {
            date: item.date || "",
            btcOutput24h: sumDailyOutput,
            theoreticalPower: sumTheoreticalPower,
            power24h: sumEffectivePower,
            effectiveRate24h,
            totalMachines: sumTotalMachines,
            totalFailures: sumTotalFailures,
            failures24h: sumNewFailures,
            failureRate24h,
            impactRatio: 0,
            onlineRatio: 0,
            limitImpactRate: 0,
