...
const WaveLineCard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response: ApiResponse = await getLast30DaysLimitImpactRate(poolType, Number(venueId));

      setDates(response.data.map((item) => item.date).reverse());
      setHashValues(response.data.map((item) => item.limitImpactRate).reverse());
      ...
      series: [
        {
          ...
          data: hashValues,
          itemStyle: {
            color: "#7c3aed", //rgb(216, 70, 70) 点的颜色
          },

          // data: makeWave(0),
          lineStyle: { width: 2, color: "#7c3aed" }, // #7c3aed
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">限电影响曲线</h3>
        {/* <Radio.Group
                    value={chart.period}
                    onChange={(e) => {
