...
const EventImpactAnalysis: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white px-3 sm:px-4">
      {/* --- Unified Header Area --- */}
      <div className="border-b border-gray-200 px-2 sm:px-3 py-3 flex flex-col sm:flex-row sm:items-start sm:justify-between shrink-0 gap-3">
        <div className="flex items-start gap-2">
          <AlertTriangle className="text-orange-500 shrink-0 mt-0.5" size={24} />
          <div>
            <h1 className="text-xl font-bold text-gray-900">{t("事件管理中心")}</h1>
            <p className="text-xs text-gray-500 mt-1">{t("统一查看、跟进与复盘场地突发事件")}</p>
          </div>
        </div>
