      ...
      <Row justify="space-between" align="middle" style={{ flexWrap: "wrap", marginBottom: 16 }}>
        <Space wrap align="start">
          <Flex vertical gap="0.825rem">
            ...
            <Paragraph style={{ color: "#999" }}>{t("实时监控各数据中心的运行状态和关键指标数据")}</Paragraph>
          </Flex>
        </Space>
        {/* 右侧统计数据 */}
        <Space
        ...
        >
          <Flex justify="space-between" gap="0.725rem">
            <Statistic
              title={<span className="title">{t("总场地数")}</span>}
              value={venueNum}
            ...
            <Statistic
              // title="子账户数"
              title={<span className="title">{t("子账户数")}</span>}
              value={subAccountNum}
